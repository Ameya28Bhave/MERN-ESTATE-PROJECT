...
export default function CreateListing() {
    ...
    const handleImageSubmit = (e) => {
        if (files.length > 0 && files.length + formData.imageUrls.length < 7){
            ...
            setImageUploadError('Image upload failed (2 MB pax per img');
            setUploading(false);
        });
        } else {
        ...
        return new Promise((resolve , reject) => {
            ...
            uploadTask.on(
                ...
                (snapshot) => {
                    ...
                    console.log(`Upload is in ${progress}% done`);
                },
                (error) => {
                    reject(error);
                },
    ...
    <main className="p-3 max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex flex-col flex-1 gap-4">
            ...
            <span className="font-normal text-gray-600 ml-2">The first image will be teh cover (max 6)</span>
            </p>
            ...
            <button disabled={loading || uploading} className="p-3 bg-slate-700 text-white rounded-lg uppercase hover:opacity-95 disabled:opoacity-80">{loading ? 'Creating...' : 'Create Listing'}</button>
